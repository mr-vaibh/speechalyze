...
def classify_severity(jitter, shimmer, wpm, pauses):
    """Classify the severity of stuttering based on jitter, shimmer, WPM, and pauses."""
...
def generate_balanced_data(num_samples):
    categories = {
        "No Stutter": (0, 0.5, 0, 1, 120, 150, 0, 1),
        "Low Stutter": (0.5, 1, 1, 2, 100, 120, 1, 2),
        "Mild Stutter": (1, 2, 2, 3, 85, 100, 2, 4),
        "Medium Stutter": (2, 3, 3, 4, 60, 85, 3, 5),
        "High Stutter": (3, 4, 4, 6, 40, 60, 5, 10),
        "Severe Stutter": (4, 5.5, 6, 7.5, 20, 40, 10, 15)
    }
    ...
    return pd.DataFrame(data, columns=["Jitter (%)", "Shimmer (%)", "WPM (Words per Minute)", "Pauses (Duration in sec)", "Severity of Stutter"])

# Generate balanced dataset
df = generate_balanced_data(num_samples)

# Save to CSV
df.to_csv("data/synthetic_stutter_data.csv", index=False)

print("Balanced dataset generated and saved as 'speech_stutter_data.csv'")
